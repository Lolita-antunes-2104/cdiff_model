...
run_model_to_equilibrium <- function(params_vec, init_cond, time_vec) {as.data.frame(lsoda(init_cond, time_vec, cdiff_micro, params_vec))}
...
cdiff_micro <- function(t, pop, params) {
  with(as.list(c(pop, params)), {
    ...
    dI_c <- sigma_c*C0_c + sig_c$sigma_A*CA_c - epsilon*I_c - alpha_I*I_c + delta_I*I_h
    
    dS_II_c <- p*epsilon*I_c + gamma*C_II_c - lambda_c*S_II_c - phi*S_II_c - alpha*S_II_c + delta*S_II_h
    
    dC_II_c <- (1-p)*epsilon*I_c + lambda_c*S_II_c - gamma*C_II_c - sig_c$sigma_II*C_II_c - alpha*C_II_c + delta*C_II_h
    
    dI_II_c <- sig_c$sigma_II*C_II_c - epsilon*I_II_c - alpha_II*I_II_c + delta_II*I_II_h
    
    dS_III_c <- p*epsilon*(I_II_c + I_III_c) + gamma*C_III_c - lambda_c*S_III_c - phi*S_III_c - alpha*S_III_c + delta*S_III_h
    
    dC_III_c <- (1-p)*epsilon*(I_II_c + I_III_c) + lambda_c*S_III_c - gamma*C_III_c - sig_c$sigma_III*C_III_c - alpha*C_III_c + delta*C_III_h
    
    dI_III_c <- sig_c$sigma_III*C_III_c - epsilon*I_III_c - alpha_III*I_III_c + delta_III*I_III_h
