...
cdiff_vacc <- function(t, pop, params) {
  with(as.list(c(pop, params)), {
    ...
    # ---- Dynamic alpha (GLOBAL - calculated on total population) ----
    # Total hospital outflow (v + nv)
    out_hc_total <- delta * (tot_h_nv$S + tot_h_nv$C + tot_h_v$S + tot_h_v$C) +
      delta_I   * (I_h_nv + I_h_v) +
      delta_II  * (I_II_h_nv + I_II_h_v) +
      delta_III * (I_III_h_nv + I_III_h_v)
    
    # Total community denominator (v + nv)
    den_alpha_total <- w * (tot_c_nv$S + tot_c_nv$C + tot_c_v$S + tot_c_v$C) +
      w_I   * (I_c_nv + I_c_v) +
      w_II  * (I_II_c_nv + I_II_c_v) +
      w_III * (I_III_c_nv + I_III_c_v)
    
    if (den_alpha_total <= 0) den_alpha_total <- 1
    
    # Single alpha applied to both groups
    alpha <- out_hc_total / den_alpha_total
    alpha_I   <- alpha * w_I
    alpha_II  <- alpha * w_II
    ...
    dC0_c_nv <- lambda_c*S0_c_nv - gamma*C0_c_nv - tau_c*C0_c_nv + omega_c*CA_c_nv - 
      sigma_c*C0_c_nv - alpha*C0_c_nv + delta*C0_h_nv
    
    dCA_c_nv <- lambda_c*SA_c_nv - gamma*CA_c_nv + tau_c*C0_c_nv - omega_c*CA_c_nv - 
      sig_c_nv$sigma_A*CA_c_nv - alpha*CA_c_nv + delta*CA_h_nv
    ...
    dS_II_c_nv <- p*epsilon*I_c_nv + gamma*C_II_c_nv - lambda_c*S_II_c_nv - phi*S_II_c_nv - 
      alpha*S_II_c_nv + delta*S_II_h_nv + p*delta_I*I_h_nv
    
    dC_II_c_nv <- (1-p)*epsilon*I_c_nv + lambda_c*S_II_c_nv - gamma*C_II_c_nv - 
      sig_c_nv$sigma_II*C_II_c_nv - alpha*C_II_c_nv + delta*C_II_h_nv + (1-p)*delta_I*I_h_nv
    ...
    dS_III_c_nv <- p*epsilon*(I_II_c_nv + I_III_c_nv) + gamma*C_III_c_nv - lambda_c*S_III_c_nv - 
      phi*S_III_c_nv - alpha*S_III_c_nv + delta*S_III_h_nv + p*delta_II*I_II_h_nv + p*delta_III*I_III_h_nv
    
    dC_III_c_nv <- (1-p)*epsilon*(I_II_c_nv + I_III_c_nv) + lambda_c*S_III_c_nv - gamma*C_III_c_nv - 
      sig_c_nv$sigma_III*C_III_c_nv - alpha*C_III_c_nv + delta*C_III_h_nv + 
