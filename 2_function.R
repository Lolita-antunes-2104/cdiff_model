...
#   out <- deSolve::lsoda(y = init_cond, times = time_vec, func = cdiff_model_for_calibration, parms = params) # solve ODE
#   out <- as.data.frame(out)                                                                                # convert to data.frame
#   return(out)                                                                                               # return full trajectory
# }

# # Check whether the system is (approximately) at equilibrium
# # Idea: at equilibrium, all state derivatives dX/dt should be ~0 (we ignore cumulative counters)
# # This version:
# check_equilibrium <- function(pop, params, model_func, out, threshold = 1e-8) {
#   
#   # keep only non-cumulative compartments (computed once) 
#   keep_idx <- !grepl("Cum", names(pop)) # grepl() : returns TRUE if the pattern appears in the string
#   
#   # ---- 1) Check equilibrium at the end of the simulation (last row of 'out') ----
#   state_end <- as.numeric(out[nrow(out), names(pop)]) # take the last simulated state (same order as pop)
#   names(state_end) <- names(pop) # put names back
#   # Extract the derivatives vector (dydt) from the EDO of model_func
#   derivs_end <- model_func(     
#     t = out$time[nrow(out)],    # time corresponding to the last row
#     pop = state_end,            # state at the last row
#     params = params)[[1]]       # [[1]] = take the 1st element of the list returned by model_func (the dydt vector)                                         
#   
#   names(derivs_end) <- names(pop) # name derivatives like the compartments
#   abs_end <- abs(derivs_end[keep_idx]) # abs() : absolute value |dX/dt| without CumI
#   # TRUE if all |dX/dt| are below the threshold at the end
#   at_eq_end <- all(abs_end < threshold)  # all() : TRUE only if all elements are TRUE
#   
#   # ---- 2) Find the first time equilibrium is reached ----
#   t_eq <- NA_real_ # will store the first equilibrium time (NA = not found)
#   max_at_eq <- NA_real_ # will store the max |dX/dt| at that time
#   
#   # Loop over each saved time step in the simulation
#   for (i in seq_len(nrow(out))) {
#     state_i <- as.numeric(out[i, names(pop)]) # extract the state vector at row i (same compartment order)
#     names(state_i) <- names(pop) # put names back
#     
#     derivs_i <- model_func(t = out$time[i], pop = state_i, params = params)[[1]]  # dydt at time i
#     names(derivs_i) <- names(pop)                                    # name derivatives
#     
#     abs_i <- abs(derivs_i[keep_idx])                                 # |dX/dt| without Cum*
#     
#     if (all(abs_i < threshold)) {                                    # equilibrium reached?
#       t_eq <- out$time[i]                                            # save first time
#       max_at_eq <- max(abs_i)                                        # save max |dX/dt| at that time
#       break                                                          # stop at first time
#     }
#   }
#   
#   # ---- 3) Print results (only cat) ----
#   if (is.na(t_eq)) {
#     cat("Pas d'équilibre sur la fenêtre du burn-in\n")
#   } else {
#     cat("Équilibre atteint à t =", t_eq, "jours (max |dX/dt| =", max_at_eq, ")\n")
#   }
#   
# }